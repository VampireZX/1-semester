...
public:
    ...
    void show()
    {
        cout << "|" << godrozhd << "|\t|" << ngruppib << "| |" <<
            phis << "|" << mat << "|" << inf << "|" << xim << "| |" << srball() << "|\t" << "|" << imya << "|" << endl;
    }
  ...
  /* 
   a[0].imya = "Гуцев В.В";
    ...
    while (!in.eof()) {
        ...
        in >> a[i].godrozhd >> a[i].ngruppib >> a[i].phis >> a[i].mat >> a[i].inf >> a[i].xim;
        getline(in, a[i].imya);
        i++;
    }
    ...
    while (true)
    {
        ...
        case 3:
            ...
            for (int i = 0; i < k + n; i++)
            {
                ...
                if ((a[i].mat == 8 || a[i].mat == 9) && a[i].imya < " Baaaa") {
                    out << a[i].godrozhd << " " << a[i].ngruppib << " " << a[i].phis<< " " << a[i].mat<< " " << a[i].inf<< " " << a[i].xim<< " " << " " << a[i].imya;
