...
Start:
	...
	{
	case 1:
		InputTime(key, "Введите нужное время прибытия:\n");
		cout << "\n";
...
{
	InputNumber(table.number, "Номер: ", 1);
	cout << "Пункт назначения: ";
	getline(cin, table.destination);
	InputTime(table.timeDeparture, "Время отправления:\n");
	InputTime(table.timeArrival, "Время прибытия:\n");
	cout << "\n";
}

void ShowTableInfo(Table table)
{
	cout << "Номер: " << table.number << "\n";
	cout << "Пункт назначения: " << table.destination << "\n";
	ShowTime(table.timeDeparture, "Время отправления: ");
	cout << "\n";
	ShowTime(table.timeArrival, "Время прибытия: ");
	cout << "\n\n";
}
